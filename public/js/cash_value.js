    ...
    cortes.map(function(value){
        $('#lista_cortes').append(`<tr>
            ...
            <td>
                ...
                <input type="number" name="quantity[]" id="input-cash-${value.replace('.', '-')}" min="0" step="1" style="width:100px" data-value="${value}" class="form-control input-corte" value="0" required>
            </td>
            <td><label class="label-money" id="label-${value.replace('.', '')}">0.00 Bs.</label><input type="hidden" class="input-subtotal" id="input-${value.replace('.', '')}"></td>
        </tr>`);
    });
